...
Re-apply GP calibration + VOR + Edge on the committed board without a regenerate: `npm run gp:recalibrate` (idempotent — recalibrates from `modelGamesPlayed`, then re-applies the rate calibration).
