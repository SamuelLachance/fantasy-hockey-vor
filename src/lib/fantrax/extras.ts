/**
 * Optional data the Fantrax player table merges into its rows:
 *
 * - `public/fantrax/dynasty.json`: a dynasty value per Fantrax id (value,
 *   career phase, ETA, P(NHL), p10/p50/p90). Only fetched when the build
 *   saw the file (no pipeline publishes it yet);
 * - Simon « Snake » Boisvert's verdicts: the compact `snake/fantrax.json`
 *   (loaded with every Captains page) or `snake/nhl.json`, and the full
 *   `snake/index.json` (projection, opinion counts) only when asked for.
 *
 * The files come from other pipelines, so the readers here are tolerant:
 * a few spellings of each field are accepted, unknown fields are ignored,
 * and the table only shows the columns and filters whose field actually
 * appears in the file.
 */
