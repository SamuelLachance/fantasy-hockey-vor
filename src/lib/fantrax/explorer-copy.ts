/**
 * French copy for the /league player explorer (labels, cell text, notes),
 * kept out of the components so it is testable without a browser. Numbers
 * go through the same hand-built formatters as the rest of the page.
 */
import { FANTRAX_ICON } from "./config";
import type { ColumnKey, ExplorerRow, ExplorerType, SortKey } from "./explorer";
import { fmtNum, fmtOdds, pickLabel, plural } from "./league-copy";
...
export const TYPE_LABEL: Record<ExplorerType, string> = {
  tous: "Tous",
  proj: "Projetés",
  espoirs: "Espoirs",
};
...
export function sortButtonLabel(label: string, active: boolean, dir: "asc" | "desc"): string {
  if (!active) return `Trier par ${label}`;
  return `${label}, tri ${dir === "asc" ? "croissant" : "décroissant"} (cliquer pour inverser)`;
}

/** `2026 · 23e · DET` */
export function nhlDraftLabel(d: ExplorerRow["nhlDraft"]): string {
  if (!d) return "—";
...
export function statusCopy(
  row: Pick<ExplorerRow, "owner" | "free" | "rosterStatus">,
...
export function sourceLabel(src: ExplorerRow["src"]): string {
  return src === "p" ? "Projeté" : src === "e" ? "Espoir" : "Sans projection";
}

/** `2650` → `2 650` (built by hand, like every number on the page). */
...
/** `1 234 joueurs · page 2 sur 25` (the result counter). */
...
/** `trié par Valeur, décroissant` (next to the counter: the sort is visible even when its column is not). */
export function sortSummary(sort: { key: SortKey; dir: "asc" | "desc" }): string {
  return `trié par ${SORT_LABEL[sort.key]}, ${sort.dir === "asc" ? "croissant" : "décroissant"}`;
}

/** The counter line: results, then the sort. */
export function counterText(total: number, page: number, pages: number, sort: { key: SortKey; dir: "asc" | "desc" }): string {
...
export function explorerNote(opts: {
  draftOpen: boolean;
  ...
  const parts = [
    ...
    `Valeur = points projetés sur la saison, jusqu'à +50${NBSP}% si vos postes D ou G sont vides (comme au repêchage).`,
  ];
  if (opts.draftOpen && opts.nextPick !== null) {
    parts.push(
      `VONA et Dispo. (chance d'être encore là à votre choix ${pickLabel(opts.nextPick)}) : mêmes calculs que le panneau Repêchage, pour les joueurs projetés; les choix faits en direct retirent les joueurs repêchés.`,
    );
  }
  parts.push("Les espoirs n'ont pas de projection : triez-les par % Fantrax, ADP, âge ou rang au repêchage de la LNH.");
  if (opts.dynasty) parts.push("Valeur dynastie : modèle à long terme publié avec le site.");
