...
export function projectGoalieRatesFromRows(
  ...
  const ewmaSavePct = weightedRate(seasons, (s) => {
    const sv = s.stats.savePct ?? LEAGUE_SV_PCT;
    return sv > 0 ? sv : LEAGUE_SV_PCT;
  });
  ...
  let winRate =
    ewmaWinRate > 0
      ...
      : skill.winRate > 0
        ? skill.winRate
        : LEAGUE_WIN_RATE;
  winRate = Math.min(MAX_WIN_RATE, winRate * ageMult * teamBoost * skillWinMult);
  ...
  const teamSvEnv = teamAdjustedExpectedSv(teamGa);
  ...
  return {
    ...
    saves: savesPerGame,
    savePct: Math.max(0.875, Math.min(0.93, savePct)),
  };
}
